...
const CreateInvoiceSchema = createFormSchema.omit({ id: true, date: true });
...
const UpdateInvoice = createFormSchema.omit({ id: true, date: true });

export async function updateInvoice(id: string, prevState: State, formData: FormData) {
  const UpdateFields = UpdateInvoice.safeParse({
    customerId: formData.get("customerId"),
    amount: formData.get("amount"),
    status: formData.get("status"),
  });
  ...
  const { customerId, amount, status } = UpdateFields.data;
